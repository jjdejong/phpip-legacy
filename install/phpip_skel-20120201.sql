...
CREATE DATABASE /*!32312 IF NOT EXISTS*/ `phpip` /*!40100 DEFAULT CHARACTER SET utf8 COLLATE utf8_bin */;
...
CREATE TABLE `User` (
  ...
  `PasswordSalt` varchar(100) COLLATE utf8_bin NOT NULL,
  `LastLogin` datetime NOT NULL,
  `IP` varchar(15) COLLATE utf8_bin NOT NULL,
  `IPNum` int(10) NOT NULL,
  `Browser` varchar(100) COLLATE utf8_bin NOT NULL,
  `IPCity` varchar(255) COLLATE utf8_bin NOT NULL,
...
CREATE TABLE `actor` (
  ...
  `tmpcode` varchar(10) DEFAULT NULL COMMENT 'Delete after migration',
  PRIMARY KEY (`ID`),
...
) ENGINE=InnoDB AUTO_INCREMENT=2114 DEFAULT CHARSET=utf8 COMMENT='All actors involved in a patent';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=6482 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
...
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8 COMMENT='Indicate which actors should be systematically added to new ';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=23787 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `event_after_insert` AFTER INSERT ON `event` FOR EACH ROW trig: BEGIN
  ...
  create_tasks: LOOP
	...
	IF (vunique OR NEW.code='PRI') THEN -- Tasks triggered by priority events are also considered unique
	...
	-- Past renewals are due for divisionals, set their due date from the divisional filing date for past renewals, which should be the currently inserted event
  ...
  IF NEW.code = 'FIL' THEN
	...
	recalc_linked: LOOP
		...
		CALL recalculate_tasks_from_filed(vid);
  ...
  IF NEW.code IN ('PRI', 'PFIL') THEN
    ...
    CALL recalculate_tasks_from_filed(NEW.matter_ID);
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `event_after_update` AFTER UPDATE ON `event` FOR EACH ROW trig: BEGIN
  ...
  -- Cursor for finding the rules applicable to the updated event for recalculating the corresponding tasks
  ...
  -- Cursor for finding the matters linking to the current matter via alt_matter_id, i.e. the matters having tasks that depend on the event being changed here (currently only the Filed event)
  DECLARE cur_linked CURSOR FOR
	SELECT matter_id FROM event WHERE alt_matter_ID=NEW.matter_ID;
  ...
  -- Leave trigger if the event date information has not changed
...
-- Assigns all identified tasks to the new default responsible, except those that were not assigned to the old default responsible
...
) ENGINE=InnoDB AUTO_INCREMENT=3680 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `matter_after_insert` AFTER INSERT ON `matter` FOR EACH ROW BEGIN
	...
	-- Insert default actors defined in default_actor. Use only those that have for_client=NULL, since the non NULL values will be used in a trigger of matter_actor_lnk on a per client basis
	...
	-- (Ensure that the actor is only inserted in a container if shared=1)
...
-- Updates the tasks assigned to a user to reflect the change of the matter's responsible
...
-- Delete renewal tasks upon adding 'CLIENT' as the annuity agent
...
) ENGINE=InnoDB AUTO_INCREMENT=91264 DEFAULT CHARSET=utf8 COMMENT='Tasks triggered by events from country law information';
/*!40101 SET character_set_client = @saved_cs_client */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
...
) ENGINE=InnoDB AUTO_INCREMENT=234 DEFAULT CHARSET=utf8 COMMENT='Rules for calculating tasks from events';
/*!40101 SET character_set_client = @saved_cs_client */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trules_after_update` AFTER UPDATE ON `task_rules` FOR EACH ROW BEGIN
	-- Update new cost and fee in future tasks
...
/*!50106 DROP EVENT IF EXISTS `expire_update` */;
DELIMITER ;;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;;
...
/*!50106 CREATE EVENT `expire_update` ON SCHEDULE EVERY 1 DAY STARTS '2011-10-10 00:00:00' ON COMPLETION NOT PRESERVE DISABLE ON SLAVE DO UPDATE matter SET dead = 1 WHERE expire_date <= Now() and dead = 0 */ ;;
...
proc: BEGIN
  ...
  SELECT ID, event_date INTO vfiled_id, vfiled_date FROM event WHERE matter_ID=Pmatter_ID AND code='FIL';
  IF vfiled_id IS NULL THEN
    -- No filing event - leave
    LEAVE proc;
  END IF;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `matter_status_list` AS select `e1`.`matter_ID` AS `matter_id`,`event_name`.`name` AS `status`,`e1`.`event_date` AS `status_date` from ((`event` `e1` join `event_name` on(((`event_name`.`code` = `e1`.`code`) and (`event_name`.`status_event` = 1)))) left join `event` `e2` on(((`e1`.`matter_ID` = `e2`.`matter_ID`) and (`e1`.`event_date` < `e2`.`event_date`)))) where isnull(`e2`.`matter_ID`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2012-02-01 11:05:59
-- MySQL dump 10.13  Distrib 5.1.41, for debian-linux-gnu (x86_64)
--
-- Host: localhost    Database: phpip
-- ------------------------------------------------------
...
INSERT INTO `matter_category` VALUES ('AGR',NULL,'Agreement','OTH','root@localhost','2011-09-27 15:52:36','root'),('DSG',NULL,'Design','TM','root@localhost','2011-09-27 15:26:59','root'),('IN',NULL,'Register Change','OTH','root@localhost','2011-09-27 15:52:04','root'),('ISS','300','Issue','LTG','root','2011-10-20 16:15:34',NULL),('LTG','300','Litigation','LTG','root@localhost','2011-10-13 16:18:28','root'),('OPI','300','Legal opinion','LTG','root','2011-10-20 16:15:41','root'),('OPO','300','Opposition','LTG','root@localhost','2011-10-20 16:14:10','root'),('OTH','300','Others','OTH','root','2011-10-13 16:18:36','root'),('PAT','100','Patent','PAT','root@localhost','2011-09-27 15:44:31','root'),('PRO','100','Provisional Application','PAT','root@localhost','2011-10-13 16:19:03','root'),('S6','6MO','6 mo Watch','OTH','root@localhost','2012-01-22 17:02:29','root'),('SO',NULL,'Soleau Envelop','PAT','root@localhost','2011-09-27 15:26:20','root'),('SR','300','Search','LTG','root','2011-10-20 16:28:52',NULL),('TM','200','Trade Mark','TM','root@localhost','2011-09-27 15:44:36','root'),('TS','TS-','Trade Secret','PAT','root@localhost','2012-01-22 17:34:19','root'),('UC','100','Utility Certificate','PAT','root@localhost','2011-10-13 16:18:58','root'),('UM',NULL,'Utility Model','PAT','mdescaves','2011-11-23 14:47:55',NULL);
...
INSERT INTO `task_rules` VALUES (1,1,'PRID','FIL',0,0,'PAT',NULL,NULL,NULL,NULL,0,12,0,0,0,'PRI',NULL,1,0,NULL,NULL,NULL,NULL,NULL,NULL,'Priority deadline is inserted only if no priority event exists','root@localhost','2011-11-04 08:03:27','root'),(2,1,'PRID','FIL',0,0,'TM',NULL,NULL,NULL,NULL,0,6,0,0,0,'PRI',NULL,0,0,NULL,NULL,NULL,NULL,NULL,NULL,'Priority deadline is inserted only if no priority event exists','root@localhost','2011-11-04 08:03:27','root'),(3,1,'FBY','FIL',1,0,'PAT',NULL,NULL,NULL,NULL,0,0,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,NULL,NULL,'Clear \"File by\" task when \"Filed\" event is created','root@localhost','2011-11-04 07:23:08','root'),(4,1,'PRID','FIL',0,0,'PRO',NULL,NULL,NULL,NULL,0,12,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(5,1,'DBY','DRA',1,0,'PAT',NULL,NULL,NULL,NULL,0,0,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,NULL,NULL,'Clear \"Draft by\" task when \"Drafted\" event is created','root@localhost','2011-11-04 07:23:01','root'),(6,1,'REQ','FIL',0,0,'PAT','JP',NULL,NULL,NULL,0,0,3,0,0,'EXA',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(7,1,'REQ','PUB',0,0,'PAT','EP',NULL,NULL,NULL,0,6,0,0,0,'EXA',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(8,1,'EXP','FIL',0,0,'PRO',NULL,NULL,NULL,NULL,0,12,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(9,1,'REP','SR',0,0,'PAT','FR',NULL,NULL,'Search Report',0,3,0,0,0,'GRT',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-03-22 21:00:57','root@localhost'),(10,1,'REP','EXA',0,0,'PAT',NULL,NULL,NULL,'Exam Report',0,3,0,0,0,'GRT',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-11-23 14:00:45','mdescaves'),(11,1,'REP','EXA',0,0,'PAT','EP',NULL,NULL,'Exam Report',0,4,0,0,0,'GRT',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-03-20 11:17:37','root@localhost'),(12,1,'EXP','FIL',0,0,'PAT',NULL,NULL,NULL,NULL,0,0,20,0,0,NULL,NULL,1,0,NULL,NULL,NULL,NULL,'EUR',NULL,'Create the \"Expires\" event','root@localhost','2011-09-02 14:13:42','root'),(13,1,'REP','ALL',0,0,'PAT','EP',NULL,NULL,'R71(3)',0,4,0,0,0,'GRT',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-12-05 13:07:47','root'),(14,1,'PAY','ALL',0,0,'PAT','EP',NULL,NULL,'Grant fee',0,4,0,0,0,'GRT',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-11-23 14:13:54','mdescaves'),(15,1,'PROD','ALL',0,0,'PAT','EP',NULL,NULL,'Claim Translation',0,4,0,0,0,'GRT',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-03-20 11:18:32','root@localhost'),(16,1,'VAL','GRT',0,0,'PAT','EP',NULL,NULL,'Translate where necessary',0,3,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-12-05 15:58:17','mdescaves'),(17,1,'FDIV','EXA',0,0,'PAT','EP',NULL,NULL,NULL,0,0,2,0,0,'GRT',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(18,1,'REP','PUB',0,0,'PAT','EP',NULL,NULL,'Written Opinion',0,6,0,0,0,'EXA',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-03-20 15:34:56','root@localhost'),(19,1,'PAY','PUB',0,0,'PAT','EP',NULL,NULL,'Designation Fees',0,6,0,0,0,'EXA',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-03-20 15:34:58','root@localhost'),(20,1,'PROD','PRI',0,0,'PAT','US',NULL,NULL,'Decl. and Assignment',0,12,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-12-05 13:08:15','root'),(21,1,'FBY','PRI',0,0,'PAT',NULL,NULL,NULL,'Priority Deadline',0,12,0,0,0,'FIL',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-11-10 10:22:11','root'),(22,1,'NPH','FIL',0,0,'PAT','WO',NULL,NULL,NULL,0,30,0,0,0,NULL,NULL,0,1,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(23,1,'REQ','PRI',0,0,'PAT','WO',NULL,NULL,NULL,0,22,0,0,0,NULL,NULL,0,1,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-11-28 10:58:15','mdescaves'),(24,1,'DBY','REC',0,0,'PAT',NULL,NULL,NULL,NULL,0,2,0,0,0,'PRI',NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-10-21 07:33:15','root'),(25,1,'PRID','PRI',0,1,'PAT',NULL,NULL,NULL,NULL,0,0,0,0,0,NULL,'FIL',0,0,NULL,NULL,NULL,NULL,'EUR',NULL,'Deletes priority deadline when a priority event is inserted','root@localhost','2011-11-04 08:03:27','root'),(26,1,'EHK','PUB',0,0,'PAT','CN',NULL,NULL,NULL,0,6,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(27,1,'FOP','GRT',0,0,'OP','EP',NULL,NULL,NULL,0,9,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(28,1,'ABO','GRT',0,0,'PAT','EP',NULL,NULL,NULL,0,9,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(29,1,'DBY','FIL',1,0,'PAT',NULL,NULL,NULL,NULL,0,0,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(30,1,'PROD','FIL',0,0,'PAT','US',NULL,NULL,'IDS',0,2,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-03-22 20:09:57',NULL),(31,1,'REM','FIL',0,0,'PAT','FR',NULL,NULL,'Translation for Revision',0,6,0,0,0,'PRI',NULL,1,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-10-12 15:33:07','root'),(32,1,'REP','EXA',0,0,'PAT','WO',NULL,NULL,NULL,0,3,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-09-02 14:13:42','root'),(33,1,'EXP','FIL',0,0,'PAT','WO',NULL,NULL,NULL,0,31,0,0,0,NULL,NULL,0,1,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-09-02 14:13:42','root'),(34,1,'REM','FIL',0,0,'PAT','WO',NULL,NULL,'National Phase',0,27,0,0,0,NULL,NULL,0,1,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-05-09 17:11:57','root@localhost'),(35,1,'PROD','FIL',0,0,'PAT','FR',NULL,NULL,'Small Entity',0,1,0,0,0,'PRI',NULL,1,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-10-05 19:56:19','root'),(36,1,'PAY','GRT',0,0,'PAT','CN',NULL,NULL,'HK Grant Fee',0,6,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root@localhost','2011-03-22 22:59:22',NULL),(37,1,'REP','COM',0,0,'PAT',NULL,NULL,NULL,'Communication',0,2,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR','Check deadline for responding',NULL,'root@localhost','2011-08-11 09:05:44','root'),(38,1,'FOP','OPP',1,0,'OP','EP',NULL,NULL,NULL,0,0,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,'Clear \"File Opposition\" task when \"Opposition\" event is created','root@localhost','2011-09-02 14:13:42','root'),(39,1,'PAY','ALL',0,0,'PAT','JP',NULL,NULL,'Grant fee',0,1,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-11-23 14:14:59','mdescaves'),(40,1,'FPR','DW',0,0,'PAT','EP',NULL,NULL,NULL,0,2,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-10-09 22:41:20','root'),(41,1,'REP','PUB',0,0,'PAT','EP',NULL,NULL,'R70(2)',0,6,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2011-10-11 09:22:24','root'),(42,1,'NPH','PRI',0,0,'PAT',NULL,'WO',NULL,NULL,0,30,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-10-14 07:20:28',NULL),(43,1,'PAY','FIL',0,0,'PAT','FR',NULL,NULL,'Filing fees',0,1,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,'536.00','EUR',NULL,NULL,'root','2011-10-14 12:57:36','root'),(44,1,'PAY','FIL',0,0,'PAT','EP',NULL,NULL,'Filing fees',0,1,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,'1210.00','EUR',NULL,NULL,'root','2011-10-14 12:57:27',NULL),(45,1,'VAL','GRT',0,0,'PAT',NULL,'EP',NULL,NULL,0,3,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-10-17 13:42:58',NULL),(46,1,'REP','RSTR',0,0,'PAT','US',NULL,NULL,'Restriction Req.',0,1,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-11-03 15:58:40',NULL),(47,1,'REP','COM',0,0,'PAT','EP',NULL,NULL,'R161',0,6,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2011-10-19 08:23:13','root'),(48,1,'NAP','REF',0,0,'PAT',NULL,NULL,NULL,NULL,0,2,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-11-03 16:38:31',NULL),(49,1,'GAP','REF',0,0,'PAT',NULL,NULL,NULL,NULL,0,4,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-11-03 16:39:48',NULL),(50,1,'OPR','SOP',0,0,'PAT','EP',NULL,NULL,NULL,0,3,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-11-03 17:11:27',NULL),(51,1,'PROD','SOP',0,0,'PAT','EP',NULL,NULL,'Comments',0,0,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-11-04 07:46:45','root'),(52,1,'REP','COM',0,0,'OP','EP',NULL,NULL,'Observations',0,4,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2011-11-03 18:06:54',NULL),(53,1,'REQ','FIL',0,0,'PAT','KR',NULL,NULL,NULL,0,0,5,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2011-12-13 13:37:48','root'),(54,1,'REQ','FIL',0,0,'PAT','CA',NULL,NULL,NULL,0,0,5,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2011-12-13 13:37:55','root'),(55,1,'REQ','PRI',0,0,'PAT','CN',NULL,NULL,NULL,0,0,3,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2011-12-13 13:38:01','root'),(56,1,'PAY','ALL',0,0,'PAT','CA',NULL,NULL,'Grant Fees',0,6,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2011-12-13 13:38:06','root'),(57,1,'PROD','PRI',0,0,'PAT',NULL,NULL,NULL,'Priority Docs',0,16,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2011-12-13 13:38:12','root'),(58,1,'PAY','FIL',0,0,'PAT','WO',NULL,NULL,'Filing Fees',0,1,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2011-12-13 13:38:17','root'),(59,1,'PROD','FIL',0,0,'PAT','WO',NULL,NULL,'Power',0,1,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2011-12-13 13:38:25','root'),(60,1,'REM','ALL',0,0,'PAT','US',NULL,NULL,'File divisional',0,1,0,0,0,NULL,'RSTR',0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'root','2012-01-19 09:50:59',NULL),(61,1,'REP','EXA',0,0,'PAT','CN',NULL,NULL,'Exam Report',0,4,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2012-01-19 09:51:38','root'),(62,1,'REP','EXA',0,0,'PAT','CA',NULL,NULL,'Exam Report',0,6,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2012-01-19 09:51:51','root'),(63,1,'REM','SR',0,0,'PAT','FR',NULL,NULL,'Request extension',0,3,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2012-01-19 09:52:04','root'),(64,1,'REM','EXA',0,0,'PAT','EP',NULL,NULL,'Request extension',0,4,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2012-01-19 09:52:16','root'),(65,1,'FBY','REC',0,0,'PAT',NULL,NULL,NULL,NULL,0,3,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2012-01-19 09:52:27','root'),(66,1,'PAY','ALL',0,0,'PAT','FR',NULL,NULL,'Grant Fees',0,2,0,0,0,NULL,NULL,0,0,NULL,NULL,NULL,NULL,'EUR',NULL,NULL,'mdescaves','2012-01-19 09:52:40','root'),(102,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'2',0,0,1,0,1,NULL,NULL,1,0,NULL,NULL,'36.00','70.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:04:04','root@localhost'),(103,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'3',0,0,2,0,1,NULL,NULL,1,0,NULL,NULL,'36.00','70.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:03:40',NULL),(104,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'4',0,0,3,0,1,NULL,NULL,1,0,NULL,NULL,'36.00','70.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:04:52',NULL),(105,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'5',0,0,4,0,1,NULL,NULL,1,0,NULL,NULL,'36.00','70.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:05:57',NULL),(106,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'6',0,0,5,0,1,NULL,NULL,1,0,NULL,NULL,'72.00','95.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:06:52',NULL),(107,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'7',0,0,6,0,1,NULL,NULL,1,0,NULL,NULL,'92.00','95.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:07:31',NULL),(108,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'8',0,0,7,0,1,NULL,NULL,1,0,NULL,NULL,'130.00','95.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:08:18',NULL),(109,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'9',0,0,8,0,1,NULL,NULL,1,0,NULL,NULL,'170.00','95.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:09:11',NULL),(110,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'10',0,0,9,0,1,NULL,NULL,1,0,NULL,NULL,'210.00','95.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:09:59',NULL),(111,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'11',0,0,10,0,1,NULL,NULL,1,0,NULL,NULL,'250.00','125.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:10:47',NULL),(112,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'12',0,0,11,0,1,NULL,NULL,1,0,NULL,NULL,'290.00','125.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:11:33',NULL),(113,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'13',0,0,12,0,1,NULL,NULL,1,0,NULL,NULL,'330.00','125.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:12:19',NULL),(114,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'14',0,0,13,0,1,NULL,NULL,1,0,NULL,NULL,'380.00','125.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:13:16',NULL),(115,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'15',0,0,14,0,1,NULL,NULL,1,0,NULL,NULL,'430.00','125.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:14:51','root@localhost'),(116,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'16',0,0,15,0,1,NULL,NULL,1,0,NULL,NULL,'490.00','165.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:14:43',NULL),(117,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'17',0,0,16,0,1,NULL,NULL,1,0,NULL,NULL,'550.00','165.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:15:35',NULL),(118,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'18',0,0,17,0,1,NULL,NULL,1,0,NULL,NULL,'620.00','165.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:16:18',NULL),(119,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'19',0,0,18,0,1,NULL,NULL,1,0,NULL,NULL,'690.00','165.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:17:11',NULL),(120,1,'REN','FIL',0,0,'PAT','FR',NULL,NULL,'20',0,0,19,0,1,NULL,NULL,1,0,NULL,NULL,'760.00','165.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:18:02',NULL),(203,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'3',0,0,2,0,1,NULL,NULL,1,0,NULL,NULL,'420.00','165.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:24:23',NULL),(204,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'4',0,0,3,0,1,NULL,NULL,1,0,NULL,NULL,'525.00','180.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:25:15',NULL),(205,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'5',0,0,4,0,1,NULL,NULL,1,0,NULL,NULL,'735.00','200.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:25:59',NULL),(206,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'6',0,0,5,0,1,NULL,NULL,1,0,NULL,NULL,'945.00','225.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:26:39',NULL),(207,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'7',0,0,6,0,1,NULL,NULL,1,0,NULL,NULL,'1050.00','250.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:27:18',NULL),(208,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'8',0,0,7,0,1,NULL,NULL,1,0,NULL,NULL,'1155.00','255.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:28:06',NULL),(209,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'9',0,0,8,0,1,NULL,NULL,1,0,NULL,NULL,'1260.00','270.00','EUR',NULL,NULL,'root@localhost','2011-04-23 15:28:50',NULL),(210,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'10',0,0,9,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root@localhost','2011-09-27 11:52:15','root'),(211,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'11',0,0,10,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root','2011-09-27 12:05:04','root'),(212,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'12',0,0,11,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root','2011-09-27 12:05:09','root'),(213,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'13',0,0,12,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root','2011-09-27 12:05:12','root'),(214,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'14',0,0,13,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root','2011-09-27 12:05:16','root'),(215,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'15',0,0,14,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root','2011-09-27 12:05:19','root'),(216,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'16',0,0,15,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root',NULL,NULL),(217,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'17',0,0,16,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root',NULL,NULL),(218,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'18',0,0,17,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root',NULL,NULL),(219,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'19',0,0,18,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root',NULL,NULL),(220,1,'REN','FIL',0,0,'PAT','EP',NULL,NULL,'20',0,0,19,0,1,NULL,NULL,1,0,NULL,NULL,'1420.00','300.00','EUR',NULL,NULL,'root',NULL,NULL);
...
INSERT INTO `actor` VALUES (119,'Client handled',NULL,'CLIENT',NULL,'ANN','',NULL,NULL,NULL,0,NULL,0,'',NULL,'',NULL,'',NULL,NULL,NULL,NULL,NULL,'0','DO NOT DELETE - Special actor used for removing renewal tasks that are handled by the client',NULL,'root','2011-12-14 16:26:16','root','CLIENT');
